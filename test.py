...
tests.append([r'\ln{5}                      ', math.log(5), 'natural logarithm'])
tests.append([r'\log 100                    ', math.log10(100), 'log_10'])
...
for test in tests:
    if latexcalc.calc(test[0]) == test[1]:
        ...
        print('FAIL: '+test[0]+' # result: '+str(latexcalc.calc(test[0])))
        fail = True
if fail:
    print('!!! TEST FAILED !!!')
    exit(1)
...
for test in tests:
    if latexcalc.calc(test[0], test[1]) == test[2]:
        ...
        print('FAIL: '+test[0]+' # result: '+str(latexcalc.calc(test[0], test[1])))
        fail = True
if fail:
    print('!!! TEST FAILED !!!')
    exit(1)
